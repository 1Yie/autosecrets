...
Uses the age identity from the identity directory for verification and
enrollment; run from the venv in agent/.venv or the signed artifact.
"""
...
def _enroll(config: AgentConfig, token: str, server_url: str) -> int:
    ...
    age_public = (
        AgePrivateKey.from_private_string(age_private).public_key().public_string()
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    verify = sub.add_parser(
        "verify-envelope", help="verify an envelope and print its plaintext"
    )
    verify.add_argument("envelope_file")
    verify.add_argument(
        "--signing-public", required=True, help="base64 Ed25519 public key"
    )
    verify.add_argument(
        "--recipient-private",
        required=True,
        help="age secret key (AGE-SECRET-KEY-1...)",
    )

    def add_config_args(p: argparse.ArgumentParser) -> None:
        p.add_argument("--config", default="/etc/autosecrets-agent/config.toml")
        p.add_argument(
            "--server", default="", help="override server URL (used by the installer)"
        )
        p.add_argument(
            "--token", default="", help="one-time enrollment token (enroll only)"
        )
    ...
    if args.command == "verify-envelope":
        return _verify_envelope(
            args.envelope_file, args.signing_public, args.recipient_private
        )

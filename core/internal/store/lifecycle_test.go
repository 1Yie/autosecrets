...
func TestStoreLifecycle(t *testing.T) {
	...
	if err := st.CreateApplication(ctx, uuid.NewString(), "payments"); err != store.ErrDuplicate {
	...
	if _, err := st.GetApplication(ctx, uuid.NewString()); err != store.ErrNotFound {
	...
	if err := st.CreateEnvironment(ctx, uuid.NewString(), appID, "production", "standard"); err != store.ErrDuplicate {
	...
	if _, err := st.GetEnvironment(ctx, envID, uuid.NewString()); err != store.ErrNotFound {
	...
	if err := st.CreateSecretWithValue(ctx, uuid.NewString(), uuid.NewString(), appID, envID,
		"db_pass", []byte("w1"), []byte("n1"), []byte("c1")); err != store.ErrDuplicate {
	...
	if _, _, err := st.AddSecretVersion(ctx, uuid.NewString(), uuid.NewString(),
		[]byte("w"), []byte("n"), []byte("c")); err != store.ErrNotFound {
	...
	if err := st.CreateNodeGroup(ctx, uuid.NewString(), "web-tier"); err != store.ErrDuplicate {
		t.Fatalf("duplicate group must be ErrDuplicate, got %v", err)
	}
	if err := st.AddGroupMember(ctx, groupID, uuid.NewString()); err != store.ErrNotFound {
	...
	if err := st.SaveActivationPolicy(ctx, store.ActivationPolicy{
	...
	if _, err := st.CreateAssignment(ctx, uuid.NewString(), groupID, appID, envID); err != store.ErrDuplicate {
		t.Fatalf("duplicate assignment must be ErrDuplicate, got %v", err)
	}
	if _, err := st.CreateAssignment(ctx, uuid.NewString(), uuid.NewString(), appID, envID); err != store.ErrNotFound {
	...
	if _, err := st.NodeBySerial(ctx, "nope"); err != store.ErrNotFound {
	...
	if err := st.CreateAdmin(ctx, uuid.NewString(), "alice", "hash"); err != store.ErrDuplicate {
	...
	if _, err := st.SessionByID(ctx, sessionHash, time.Now().Add(2*time.Hour)); err != store.ErrNotFound {
	...
	// Audit.
	if err := st.AppendAudit(ctx, nil, store.AuditEvent{
		Actor: "alice", Action: "secret.create", Resource: secretID,
		Result: "ok", CorrelationID: "corr-1",
	}); err != nil {
		t.Fatal(err)
	}
	events, err := st.ListAudit(ctx, store.AuditFilter{Action: "secret.create", Limit: 10})
	if err != nil || len(events) != 1 || events[0].Actor != "alice" || events[0].CorrelationID != "corr-1" {
		t.Fatalf("list audit: %v %+v", err, events)
	}
	if events, err := st.ListAudit(ctx, store.AuditFilter{Limit: 5}); err != nil || len(events) != 1 {

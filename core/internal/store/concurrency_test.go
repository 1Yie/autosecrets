...
func seedSecret(t *testing.T, st *store.Store) (appID, envID, secretID string) {
	t.Helper()
...
func TestEnrollmentTokenExpired(t *testing.T) {
	...
	if _, err := st.ConsumeEnrollmentToken(ctx, tokenHash, time.Now()); err != store.ErrConflict {
...
func TestPublishEmptyDraft(t *testing.T) {
	...
	if _, err := st.Publish(ctx, uuid.NewString(), appID, envID, "admin", store.OperationReason{
		Category: "maintenance", Explanation: "empty draft must fail",
	}); err != store.ErrNoSecrets {

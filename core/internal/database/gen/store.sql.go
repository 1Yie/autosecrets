...
WHERE (actor = $1 OR $1 IS NULL)
  ...
  AND (resource = $3 OR $3 IS NULL)
  AND (outcome = $4 OR $4 IS NULL)
  AND (operation_reason_category = $5 OR $5 IS NULL)
...
const listAuditOffsetPage = `-- name: ListAuditOffsetPage :many
SELECT id, actor, action, resource, result, correlation_id,
    to_char(created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS"Z"') AS created_at,
...
type ListAuditOffsetPageParams struct {
	Actor          *string            `json:"actor"`
	Action         *string            `json:"action"`
	Resource       *string            `json:"resource"`
	Outcome        *string            `json:"outcome"`
...
type ListAuditOffsetPageRow struct {
	ID                         int64  `json:"id"`
	Actor                      string `json:"actor"`
	Action                     string `json:"action"`
	Resource                   string `json:"resource"`
...
func (q *Queries) ListAuditOffsetPage(ctx context.Context, arg ListAuditOffsetPageParams) ([]ListAuditOffsetPageRow, error) {
	rows, err := q.db.Query(ctx, listAuditOffsetPage,
		arg.Actor,
		arg.Action,
		arg.Resource,
		arg.Outcome,
	...
	defer rows.Close()
	items := []ListAuditOffsetPageRow{}
	for rows.Next() {
		var i ListAuditOffsetPageRow
		if err := rows.Scan(
			&i.ID,
			&i.Actor,
			&i.Action,

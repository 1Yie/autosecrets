...
func TestOIDCBindingLoginAndUnbindJourney(t *testing.T) {
	...
	deniedBinding := ta.do(t, "POST", "/api/v1/auth/oidc/binding", map[string]string{
		"password": "wrong-password-42", "return_to": "/dashboard/security",
	}, cookie, csrf)
	...
	started := ta.do(t, "POST", "/api/v1/auth/oidc/binding", map[string]string{
		"password": "correct-horse-42", "return_to": "/dashboard/security",
	}, cookie, csrf)
	...
	if bound.status != http.StatusFound || bound.header.Get("Location") != "/dashboard/security" {

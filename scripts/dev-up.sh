...
if [ "$LAN" = "1" ]; then
  ...
  case "$LAN_IP" in
    127.*|"") echo "cannot determine a LAN address (hostname -I)" >&2; exit 1 ;;
  esac
...
if [ -z "${GO:-}" ]; then
  for c in go /home/ichiyo/sdk/go1.26.5/bin/go /usr/local/go/bin/go; do
    if command -v "$c" >/dev/null 2>&1 || [ -x "$c" ]; then GO="$c"; break; fi
  done
fi
[ -n "${GO:-}" ] || { echo "go toolchain not found (set GO=...)" >&2; exit 1; }
...
wget -q -O /dev/null http://127.0.0.1:18080/api/v1/health 2>/dev/null || {
  echo "core failed to start; see .dev/core.log" >&2; tail -5 "$DEV/core.log" >&2; exit 1; }
...
wget -q -O /dev/null http://127.0.0.1:5199/ 2>/dev/null || {
  echo "vite failed to start; see .dev/vite.log" >&2; tail -5 "$DEV/vite.log" >&2; exit 1; }

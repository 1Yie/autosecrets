...
import {
	useAuditEvents,
	type AuditFilters,
} from "../../hooks/audit/use-audit-events";
		...
		<div className="space-y-4">
			...
			>
				...
				<label className="flex flex-col gap-1">
					...
					<Select
						...
						onValueChange={(v) =>
							setDraft({ ...draft, outcome: v === "all" ? "" : (v ?? "") })
						}
					>
				...
				<label className="flex flex-col gap-1">
					...
					<Select
						value={draft.reason_category || "all"}
						onValueChange={(v) =>
							setDraft({
								...draft,
								reason_category: v === "all" ? "" : (v ?? ""),
							})
						}
					>
			...
			{audit.items.length === 0 ? (
				...
				<Frame className="w-full">
					...
					<TablePagination noun="事件" page={audit} />
				</Frame>
			)}
		</div>
	);

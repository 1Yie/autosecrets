...
test("single-Administrator authentication and Secret delivery journey", async ({ context, page, request }) => {
  test.skip(!KEY_DIR || !BOOTSTRAP_CODE, "E2E stack not configured");

  await page.goto("/");
  await expect(page.getByRole("heading", { name: "初始化 AutoSecrets" })).toBeVisible();
  await page.getByTestId("code").fill(BOOTSTRAP_CODE);
  await page.getByTestId("organization-name").fill("E2E Organization");
  ...
  await expect(page.getByText("当前本地登录只需要用户名和密码。")).toBeVisible();
  ...
  await expect(page.getByText("用户名和密码验证后还需要动态验证码或恢复码。")).toBeVisible();
  ...
  await expect(page.getByText("已绑定 E2E Administrator")).toBeVisible();

  await logout(page);
  await expect(page.getByRole("button", { name: "使用 OpenID Connect 登录" })).toBeVisible();
  await page.getByRole("button", { name: "使用 OpenID Connect 登录" }).click();
  await expect(page.getByRole("heading", { name: "概览" })).toBeVisible();
  const oidcMe = await page.evaluate(async () => (await fetch("/api/v1/me")).json());
  ...
  const metricsBeforeLogout = await (await request.get("http://127.0.0.1:19090/metrics")).json();
  await logout(page);
  const providerCookies = await context.cookies("http://127.0.0.1:19090");
  expect(providerCookies.some((cookie) => cookie.name === "oidc_test_session")).toBe(true);
  const metricsAfterLogout = await (await request.get("http://127.0.0.1:19090/metrics")).json();
  ...
  await expect(page.getByText("Provider 配置可用，尚未绑定身份。")).toBeVisible();
  ...
  await expect(page.getByRole("heading", { name: "概览" })).toBeVisible();
  await expect(page.getByRole("button", { name: "使用 OpenID Connect 登录" })).toHaveCount(0);
...
async function passwordStep(page: Page) {
  ...
  await page.getByTestId("password").fill(PASSWORD);
  await page.getByRole("button", { name: "登录", exact: true }).click();
  await expect(page.getByRole("heading", { name: "验证第二因子" })).toBeVisible();
}
...
async function authenticatedPost(page: Page, path: string, body: Record<string, string>) {
...
async function nextTOTPCounter(previous: number) {
  while (totpCounter() <= previous) await new Promise((resolve) => setTimeout(resolve, 250));
  return totpCounter();
}
...
async function secretDeliveryJourney(page: Page) {
  ...
  await page.getByPlaceholder("节点组名称").fill("g1");
  await page.getByRole("button", { name: "创建", exact: true }).click();
  ...
  const script = await new Promise<string>((resolve, reject) => {
    https.get(`${PROXY_URL}/agent/v1/install.sh`, { ca: readFileSync(join(KEY_DIR, "agent-ca.crt")) }, (res) => {
      let data = "";
  ...
  const memberAdded = page.waitForResponse((response) =>
    response.request().method() === "POST" && /\/api\/v1\/node-groups\/[^/]+\/nodes$/.test(response.url()),
  );
  ...
  const assignmentCreated = page.waitForResponse((response) =>
    response.request().method() === "POST" && response.url().endsWith("/api/v1/assignments"),
  );
  ...
  const syncOutput = execFileSync(agent, ["sync", "--config", join(prefix, "etc", "config.toml")], {
    timeout: 60_000,
    stdio: "pipe",
  }).toString();
  const files: string[] = [];
  ...
  expect(secretFile, `sync output: ${syncOutput}; bundle files: ${files.join(", ")}`).toBeTruthy();
